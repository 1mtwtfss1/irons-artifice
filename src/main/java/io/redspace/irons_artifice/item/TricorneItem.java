...
public class TricorneItem extends BaseGeoItem {
    ...
    @SubscribeEvent
    public static void modifyFirstShot(ComposeShotEvent event) {
        if (event.getEntity().getItemBySlot(EquipmentSlot.HEAD).is(ItemRegistry.TRICORNE_HAT)) {
            MagazineContents magazineContents = MagazineContents.get(event.getGunStack());
            if (magazineContents.count() == event.getGunProfile().magazineCapacity()) {
                event.getComponents().getOrCreate(ShotComponents.DAMAGE).addModifier(new ValueModifier(DAMAGE_BUFF_PERCENT, ValueModifier.Operation.MULTIPLY_TOTAL, ValueModifier.Type.BENEFICIAL));
            }
        }
    }
}
